...
This document plans the implementation; it does not yet change runtime code,
install inference dependencies, or enable blocking.

It supersedes the runtime and artifact assumptions in Sections 11 through 18
of `13_layer2_implement.md` where those assumptions conflict with the model
that was actually exported. The dataset preparation and training history in
`15_fine_tune_implement.md` remain useful context.
